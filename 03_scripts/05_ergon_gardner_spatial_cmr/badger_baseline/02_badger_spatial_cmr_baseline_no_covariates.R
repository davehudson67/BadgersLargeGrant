...
sampled_ids <- sample(unique(bc$tattoo), 200)
bc <- bc %>%
...
config <- configureMCMC(model, monitors = c("phi", "kappa", "sigma", "PL", "dmean", "S"))
...
saveRDS(run$summary, "EG_Badgers_NoCV_200indSummary.rds")

mcmcplot(run$samples, parms = c("sigma"))
summary <- t(as.data.frame(run$summary))
