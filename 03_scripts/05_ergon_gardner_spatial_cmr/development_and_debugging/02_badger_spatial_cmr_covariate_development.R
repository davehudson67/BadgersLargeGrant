...
sampled_ids <- sample(unique(bc$tattoo), 50)
bc <- bc %>%
...
bc <- bc %>%
  ...
  mutate(# Adjust lastSecondary to refer to the last primary session for each individual
    lastSecondary = ifelse(primary == maxPrimary, lastSecondary, NA)) %>%
...
code <- nimbleCode({
  ...
  for(i in 1:N[1]){
    ...
    for(r in 1:R){ # trap
      ...
      g[i, first[i], r + 1] <- exp(-pow(D[i, r, first[i]] / sigma, kappa)) # Trap exposure
    }
  ...
  for(i in (N[1] + 1):N[2]){
    ...
    for(k in (first[i] + 1):K[i]){ # primary session
      ...
      Palive[i, k - 1] <- z[i, k - 1] * phi[k - 1] # Pr(alive in primary session k) 
      ...
      for(r in 1:R){ # trap
        ...
        g[i, k, r + 1] <- exp(-pow(D[i, r, k] / sigma, kappa)) # Trap exposure
      }
...
run$summary
saveRDS(run, "BadgersSimplifiedKnownDeathOutS.rds")
run <- readRDS("BadgersSimplifiedKnownDeathOut.rds")
