...
bc <- select(bc, -c(cub_positive, never_positive))
bc <- left_join(bc, individual_summary, by = "tattoo")
...
bc <- bc %>%
  ...
  mutate(primary = factor(year(date), levels = c("1982", "1983", "1984", "1985", "1986", "1987", 
                                                 ...
                                                 "2018", "2019", "2020"))) %>%
  dplyr::select(tattoo, date, sett, pm, socg, SG, sex, age, primary, trap_season, row_index, col_index, cub_positive, never_positive) %>%
  ungroup()
...
inf <- bc %>%
  distinct(tattoo, .keep_all = TRUE) %>%
  #mutate(inf = as.numeric(cub_positive)) %>%
  pull(cub_positive)
...
config <- configureMCMC(model, monitors = c("phi", "kappa", "sigma", "PL", "dmean"), thin = 1,
                        monitors2 = c("S"), thin2 = 20)
